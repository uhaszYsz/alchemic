...
/**
 * Fixed gathering hubs at the four factory corners: NW wood, NE stone, SW flint, SE plants.
 * Same corner mapping as factoryMaterialFromCornerSources.
 * @returns {null | string}
 */
function factoryFixedGatheringHubMaterialId(col, row) {
    if (!factoryCellOnGrid(col, row)) return null;
    const last = factoryGridCols() - 1;
...
function factoryCellResourceId(col, row) {
    ...
    if (factoryFixedGatheringHubMaterialId(col, row)) {
        return null;
    }
    const ring = [
        [-1, -1],
    ...
    for (const [dc, dr] of ring) {
        ...
        const hubId = factoryFixedGatheringHubMaterialId(nc, nr);
...
function factoryFilterBeltPath(rawPath) {
    ...
    for (const cell of rawPath) {
        ...
        if (factoryFixedGatheringHubMaterialId(cell.col, cell.row)) break;
